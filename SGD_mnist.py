...
hidden_size = 2048      # The number of nodes at the hidden layer
...
class Net(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        self.layer3 = nn.Linear(100, num_classes)                          # Non-Linear ReLU Layer: max(0,x)
...
plt.legend()
plt.savefig('./mnist_SGD.png')
plt.show()

train_acc_mnist_lpom = np.array(train_acc_list)
np.save('./result/train_acc_mnist_sgd.npy', train_acc_mnist_lpom)
test_acc_mnist_lpom = np.array(test_acc_list)
np.save('./result/test_acc_mnist_sgd.npy', test_acc_mnist_lpom)