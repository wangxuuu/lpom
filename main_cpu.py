...
for epoch in range(num_epochs):
    for i, (images, labels) in enumerate(train_loader):   # Load a batch of images with its (index, data, class)
        images = images.view(-1, 28*28).numpy()         # Convert torch tensor to Variable: change image from a vector of size 784 to a matrix of 28 x 28
...
np.save('./result/train_acc_mnist_lpom.npy', train_acc_mnist_lpom)
test_acc_mnist_lpom = np.array(test_acc_list)
np.save('./result/test_acc_mnist_lpom.npy', test_acc_mnist_lpom)